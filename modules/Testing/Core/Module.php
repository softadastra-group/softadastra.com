...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Testing') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Testing'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Testing')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Testing'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Testing')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Testing'), __DIR__ . '/views');
