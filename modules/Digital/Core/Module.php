...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Digital') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Digital'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Digital')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Digital'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Digital')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Digital'), __DIR__ . '/views');
