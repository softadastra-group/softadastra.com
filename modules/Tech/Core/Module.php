...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Tech') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Tech'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Tech')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Tech'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Tech')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Tech'), __DIR__ . '/views');
