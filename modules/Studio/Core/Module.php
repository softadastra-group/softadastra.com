...
return new class implements ModuleContract {
    ...
    {
        $path = __DIR__ . '/Config/' . strtolower('Studio') . '.php';
        if (is_file($path)) {
            $cfg = require $path;
            $current = function_exists('config') ? (array) config(strtolower('Studio'), []) : (array) ($GLOBALS['__ivi_config'][strtolower('Studio')] ?? []);
            $merged = array_replace_recursive($current, (array) $cfg);
            if (function_exists('config_set')) {
                config_set(strtolower('Studio'), $merged);
            } else {
                $GLOBALS['__ivi_config'][strtolower('Studio')] = $merged;
            }
        }
    }
    ...
    {
        \App\Controllers\Controller::addViewNamespace(strtolower('Studio'), __DIR__ . '/views');
